...
class Omniglot(data.Dataset):
    ...
    def download(self):
        ...
        for url in self.urls:
            ...
            logging.info("== Unzip from " + file_path + " to " + file_processed)
...
def find_classes(root_dir):
    ...
    logging.info("== Found %d items " % len(retour))
    return retour
...
def index_classes(items):
    ...
    for i in items:
        if i[1] not in idx:
            idx[i[1]] = len(idx)
    logging.info("== Found %d classes" % len(idx))
    return idx
...
class OmniglotNShot:
    ...
    def __init__(self, root, batchsz, n_way, k_shot, k_query, imgsz, device=None):
        ...
        if not os.path.isfile(os.path.join(root, 'omniglot.npy')):
            ...
            logging.info(f'data shape: {self.x.shape}')  # [1623, 20, 84, 84, 1]
            temp = []  # Free memory
            # save all dataset into npy file.
            np.save(os.path.join(root, 'omniglot.npy'), self.x)
            logging.info('write into omniglot.npy.')
        else:
            # if data.npy exists, just load it.
            self.x = np.load(os.path.join(root, 'omniglot.npy'))
            logging.info('load from omniglot.npy.')
        ...
        self.dataset_sizes = {"train": len(self.x_train), "test": len(self.x_test)}
        logging.info(f"DB: train {self.x_train.shape} test {self.x_test.shape}")
    ...
    def normalization(self):
        ...
        # logging.info("before norm:", "mean", self.mean, "max", self.max, "min", self.min, "std", self.std)
    ...
    # logging.info("after norm:", "mean", self.mean, "max", self.max, "min", self.min, "std", self.std)
    ...
    def load_data_cache(self, data_pack):
        ...
        # logging.info('preload next 50 caches of batchsz of batch.')
